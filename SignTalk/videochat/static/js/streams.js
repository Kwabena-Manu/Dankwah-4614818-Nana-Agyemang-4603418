...
var mymodal = document.querySelector("#popup-modal");
...
var usernameInput = document.querySelector("#patientname")
var doctorid = document.querySelector("#doctorid")
var btnJoin = document.querySelector("#btn-join")
var transButton = document.querySelector("#trans-btn")
var photo = document.querySelector("#server-frame");
...
const el = document.getElementById('messages')
    ...
    function sendbtn() {
        ...
        if(copiedtext.trim() != ''){
            ...
            <div class="messageDiv"><span class="px-4 py-2 messageSpan rounded-lg break-words inline-block bg-blue-600 text-white ">${copiedtext}<span class="text-white-400 pl-1" style="font-size: 12px;"> <p class="text-right font-font-semibold">${currentdate.getHours()}:${currentdate.getMinutes()}</p></span></span></div>
...
//         if(action == 'new-peer'){
//             drName = parsedData['message']['drName']
//             console.log("Doctor Name: ", drName)

//             document.querySelector("#doctorName").innerHTML = drName;
//         }
//         return
//     }
...
btnJoin.addEventListener('click',(event)=>{
    ...
    switch(meetingState){
      case "not-connected":
            if(usernameInput.value.trim()!='' && doctorid.value.trim() != ''){
              connect()
...
// btnJoin.addEventListener('click', (event) => {
//     event.preventDefault()
//     if(local_call_state == call_states[0]){
...
//             sendSignal('new-peer', {'doctor':doctorid.value})
...
function connect() {
    ...
    chatSocket.addEventListener('close',(e) =>{
      if(meetingState == 'connected' || meetingState == 'connecting'){
        btnJoin.classList.remove('bg-red-700','hover:bg-red-800','focus:outline-none','text-white')
        btnJoin.classList.remove('bg-yellow-100','hover:bg-yellow-800','focus:outline-none','mr-4')
        btnJoin.classList.add('bg-gray-100','hover:bg-blue-800', 'focus:ring-4', 'focus:outline-none', 'focus:ring-blue-300')
        btnJoin.innerHTML = "Start Call"
    ...
    chatSocket.onmessage = function(evt) {
      ...
      if (msg.type == 'new-peer' &&  msg.name == myUsername){
        document.querySelector("#doctorName").innerHTML = msg.drName;
        return
      }
      switch(msg.type) {
        // case "id":
  ...
  async function invite(peerUsername, newpeerChannel) {
    ...
    //   // "preview" box (id "local_video").
      ...
      createPeerConnection();
      try {
        webcamStream = navigator.mediaDevices.getUserMedia(mediaConstraints);
...
function sendToServer(msg) {
    ...
    transButton.addEventListener("click", ()=>{
        ...
        console.log("Translate Button clicked. Transmitting status set to: ", video_trans_status)
        const FPS = 25;
        if(video_trans_status){
          sendinterval = setInterval(videotoframe,1000/FPS)
        }
        else{
          clearInterval(sendinterval)
        }
        // sendLocalVideoToServer()
    })
    
    
    // function sendLocalVideoToServer(){
    //     const FPS = 25;
    //     var trans_video_stream = document.querySelector('#local-video')
    //         setInterval(() => {
    //             var canvas = document.createElement("canvas")
    //             context = canvas.getContext('2d')
    //             width = 400;
    //             height = 300;
    //             context.drawImage(trans_video_stream, 0, 0, width, height);
    ...
    function videotoframe(){
      var trans_video_stream = document.querySelector('#local-video')
      if(trans_video_stream.srcObject && chatSocket)
      {
        var canvas = document.createElement("canvas")
        context = canvas.getContext('2d')
        width = 400;
...
function sendSignal(action, message) {
    ...
    if (action == 'new-video' && video_trans_status == false)
        return
    if (chatSocket)
...
//     console.log("Map peers:",mapPeers)
...
function dcOnMessage(event) {
    ...
    var printnow = ` <div class="flex items-end">
    <div class="flex flex-col space-y-2 text-sm font-semibold max-w-xs mx-2 order-2 items-start">
       <div><span
             class="px-4 py-2 rounded-lg inline-block break-words rounded-bl-none bg-gray-300 text-gray-600">${message}<span class="text-white-400 pl-1 " style="font-size: 12px;"> <p class="text-right font-fw-semibold">${currentdate.getHours()}:${currentdate.getMinutes()}</p></span></span></div>
                  </div>
...
function videochatRoutines(){
    endcall = document.querySelector("#trans-btn")
    console.log(endcall)
}
...
modalStatus = false
function showSignVideo(msg){
        if(modalStatus == false){
          // modal_backdrop = document.querySelector(".bg-opacity-50")
          mymodal.classList.toggle("hidden");
          // modal_backdrop.classList.toggle("hidden")
          mymodal.classList.toggle("flex");
          // modal_backdrop.classList.toggle("flex")


          modalStatus = true
        }
        var signframe = msg.message_video_frame
        console.log('serverframe',signframe)
        photo.src = signframe;
        return;
}
